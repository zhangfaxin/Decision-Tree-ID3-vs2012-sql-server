...
namespace DecisionTrees
{
    class TreeController
    {
        ...
        private static int[] isBuild = new int[6];
        ...
        {
            if (node.Name.CompareTo("Yes") == 0 || node.Name.CompareTo("No") == 0)
            {
                return false;
            }
            return true;
        ...
        public string addNewNode(string[,] dataArray, TreeNode node)
        {
            ...
            if (value < 0.1 ||end==true)//如果增益小于0.1，就直接添加叶子节点
            {
                if (积极的 > All - 积极的)
                {
                    FindNode(Form.getRoot(), Node.Tag.ToString()).Text = "Yes" + "<" + FindNode(Form.getRoot(), Node.Tag.ToString()).Tag + ">";
                    FindNode(Form.getRoot(), Node.Tag.ToString()).Name = "Yes";
                }
                else
                {
                    FindNode(Form.getRoot(), Node.Tag.ToString()).Text = "No" + "<" + FindNode(Form.getRoot(), Node.Tag.ToString()).Tag + ">";
                    FindNode(Form.getRoot(), Node.Tag.ToString()).Name = "No";
                }
        ...
        public double getEntropy_S(string[,] array, int maxFlash)
        {
            ...
            {
                if (maxFlash == flash[i]&&array[i, 4].CompareTo("Yes") == 0)
                {
                    counter++;
                 }
                i++;
        ...
        public double getEntropyByItems(string name, int maxFlash)
        {
            ...
            for (i = 0; i < size; i++)
            {
                ...
                {
                    //MessageBox.Show(Convert.ToString(i) + Convert.ToString(j)+index);
                    if (maxFlash == flash[j])
                    {
                        if (Data[j, index] != null && attribute[index][i]!=null)
                            if (Data[j, index].CompareTo(attribute[index][i]) == 0)
                            {
                                enominator[i]++;
                                if (Data[j, 4].CompareTo("Yes") == 0)
                                    numerator[i]++;
                            }
                    }
                }
